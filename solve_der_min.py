...
from base_objects import make_b_derivatives, lapAfinal2, rebuild_u, make_b_min
...
#TESTING WITH DX and DY but taking the max of the gradient each time
def solve_derivatives_min():
   ...
   target = Image.open('figs/example1/source1.jpg').resize((SCREEN_WIDTH,SCREEN_HEIGHT))
   ...
   for i,c in enumerate(source_dict.keys()):
      ...
      #A = lapAfinal2(m,n) 
      A = lapAfinal2(m,n)
      b = make_b_min(m,n,source,target)
