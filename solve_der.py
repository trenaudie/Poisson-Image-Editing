...
from base_objects import make_b_derivatives, lapAfinal2, rebuild_u
...
#TESTING WITH DX and DY first try
def solve_derivatives():
   ...
   target = Image.open('figs/possion1.png').resize((SCREEN_WIDTH,SCREEN_HEIGHT))
   source = Image.open("figs/possion1.png").resize((SCREEN_WIDTH,SCREEN_HEIGHT))
   ...
   for i,c in enumerate(source_dict.keys()):
      ...
      #A = lapAfinal2(m,n) 
      A = lapAfinal2(m,n)
      b = make_b_derivatives(m,n,source,target)
